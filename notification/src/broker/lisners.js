...
module.exports = function () {
  ...
  subscribeToQueue("PAYMENT_NOTIFICATION.PAYMENT_INITIATED", async (data) => {
    ...
    await sendEmail(
      data.email,
      "Payment Initiated",
      "Your payment has been initiated.", // fallback text
      emailTemplate, // html version
    );
  });
  ...
  subscribeToQueue("ORDER_NOTIFICATION.ORDER_CREATED", async (data) => {
    const emailTemplate = `
        ...
        <p>Dear ${data.username || "User"},</p>
        <p>A new product has been added to our collection:</p>
    ...
    await sendEmail(
      ...
      "A new product has been added to our collection.", // fallback text
      emailTemplate, // html version
    );
  });
};
